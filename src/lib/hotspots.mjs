...
export class HotspotProjector {
  constructor(parts, doorRig, host) {
    ...
    this.point = new THREE.Vector3(); this.normal = new THREE.Vector3(); this.cameraPosition = new THREE.Vector3();
    ...
    if (door) { door.geometry.computeBoundingBox(); door.geometry.boundingBox.getCenter(doorPoint); doorPoint.x = door.geometry.boundingBox.max.x + .045; }
    this.anchors = [
  ...
  update(camera, carRoot) {
    ...
    this.last = this.anchors.map(anchor => {
      ...
      this.normal.copy(anchor.normal);
      if (anchor.object) this.normal.transformDirection(anchor.object.matrixWorld);
      let reason = this.normal.dot(this.direction) < .08 ? 'back-facing' :
      ...
      const size = element?.offsetWidth || 40, margin = size / 2 + 8;
      if (!reason && (x < margin || y < margin || x > width - margin || y > height - margin)) reason = 'offscreen';
      // Keep projected targets separated instead of drawing a misleading displaced landmark.
      // Hidden details always remain available in the adjacent keyboard-accessible menu.
      if (!reason && occupied.some(other => Math.hypot(x - other.x, y - other.y) < (size + other.size) / 2 + 8)) reason = 'overlapping';
      if (!reason) occupied.push({ x, y, size });
      if (element) {
