...
export type Finish = 'rosso' | 'arancio' | 'graphite';
...
export const INITIAL_STATE: Readonly<ShowroomState> = Object.freeze({
  ...
  lights: true, fontsReady: false
});
export function isMode(value: unknown): value is Mode { return MODES.some(mode => mode === value); }
export function isFinish(value: unknown): value is Finish { return ['rosso', 'arancio', 'graphite'].includes(String(value)); }
...
export type Action =
  ...
  | { type: 'reveal' | 'replay' | 'motion' | 'inspect' | 'explode' | 'lights' | 'doors' | 'interior' | 'exit-interior' |
      ...
      'clean-photo' | 'reset-camera' | 'reset-finish' };
